...
<script type="text/javascript">
	...
	$(function(){
		...
		//创建富文本编辑器
		//orderAddEditor = TOOL.createEditor("#orderAddForm [name=file]");
		orderAddEditor = KindEditor.create("#orderAddForm [name=note]", TOOL.kingEditorParams);
		//初始化类目选择和图片上传器
		TOOL.init({fun:function(node){
			//根据订单的分类id取订单 的规格模板，生成规格信息。第四天内容。
			TOOL.changeItemParam(node, "orderAddForm");
		}});
	});
	...
	function submitOrderAddForm(){
		...
		//ajax的post方式提交表单
		//$("#orderAddForm").serialize()将表单序列化为key-value形式的字符串
		$.post("order/insert.do",$("#orderAddForm").serialize(), function(data){
			if(data.status == 200){
