...
var TOOL = {
    ...
    initPicUpload : function(data){
    	$(".picFileUpload").each(function(i,e){
        	...
        	$(e).click(function(){
        		...
        		//打开图片上传窗口
        		KindEditor.editor(TOOL.kindEditorParams).loadPlugin('multiimage',function(){
        			var editor = this;
    ...
    createEditor : function(select){
    	return KindEditor.create(select, TOOL.kindEditorParams);
    },
    ...
    initOnePicUpload : function(){
    	$(".onePicUpload").click(function(){
			var _self = $(this);
			KindEditor.editor(TOOL.kindEditorParams).loadPlugin('image', function() {
...
function formatFile(value, row, index){ 
	if(value !=null && value != ''){
		...
		for(var i in urls){
			if(urls[i] !=null && urls[i] != ''){
			resultStr +="<a href='file/download.do?fileName="+urls[i]+"'>"+urls[i].substring(urls[i].lastIndexOf("/")+1)+"</a></br></br>";
			}
		}
		return resultStr;
	}else{
